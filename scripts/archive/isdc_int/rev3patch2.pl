...
while ( $ARGV[0] ) {
	...
	foreach my $scw ( glob ( "0*" ) ) {
		if ( ( -e "./$scw/ibis_deadtime.fits.gz" ) && ( ! -l "./$scw/ibis_deadtime.fits.gz" ) ) {
		...
		if ( ( -e "ibis_deadtime.fits.gz" ) && ( -l "ibis_deadtime.fits.gz" ) ) {
			print "Deleting ibis_deadtime link\n";
			unlink "ibis_deadtime.fits.gz";
		} else {
