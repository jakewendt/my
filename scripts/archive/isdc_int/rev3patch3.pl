...
while ( $ARGV[0] ) {
	...
	foreach my $scw ( glob ( "0*" ) ) {
		...
		foreach ( "jmx1_gti", "jmx2_gti" ) {
			if ( ( -e "$_.fits.gz" ) && ( ! -l "$_.fits.gz" ) ) {
				print "$scw/$_.fits.gz already exists.  Skipping.\n";
			} else {
				unlink "$_.fits.gz" if ( -e "$_.fits.gz" );
				print "Dal Cleaning swg.fits\n";
				system ( "dal_clean  backPtrs=1 chatty=2 checkExt=0 checkSum=1 inDOL=swg.fits" );
				if ( -e "$arcscw/$_.fits.gz" ) {
					print ( "Linking and attaching $arcscw/$_.fits.gz\n" );
					system ( "ln -s $arcscw/$_.fits.gz" );
					system ( "dal_attach Parent=swg.fits Child1=$_.fits[1] Child2= Child3= Child4= Child5= " );
				} else {
					print ( "$arcscw/$_.fits.gz not found.\n" );
				}
			}
		}
